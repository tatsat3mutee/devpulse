...
import { isQuantizationSearch, QUANTIZATION_SEARCH_PATTERN } from "../search.js";
...
router.get("/archive", optionalAuth, async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (search) {
      filters.push({
        sql: (n) => isQuantizationSearch(search)
          ? `CONCAT_WS(' ', c.title, c.hook, c.mechanism, c.why_it_matters) ~* $${n}`
          : `CONCAT_WS(' ', c.title, c.hook, c.mechanism, c.why_it_matters) ILIKE $${n}`,
        value: isQuantizationSearch(search) ? QUANTIZATION_SEARCH_PATTERN : `%${search}%`,
      });
    }
