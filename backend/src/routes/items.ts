...
import {
  AI_SEARCH_CONTEXT_PATTERN,
  isQuantizationSearch,
  QUANTIZATION_SEARCH_PATTERN,
} from "../search.js";
...
router.get("/", optionalAuth, async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (search) {
      ...
      if (isQuantizationSearch(searchText)) {
        conditions.push(`CONCAT_WS(' ', i.title, i.description) ~* $${paramIdx} AND CONCAT_WS(' ', i.title, i.description) ~* $${paramIdx + 1}`);
        params.push(QUANTIZATION_SEARCH_PATTERN, AI_SEARCH_CONTEXT_PATTERN);
        paramIdx += 2;
      } else {
        conditions.push(`CONCAT_WS(' ', i.title, i.description) ILIKE $${paramIdx}`);
