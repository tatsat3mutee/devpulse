...
const navGroups: { label: string; items: NavItem[] }[] = [
  {
    label: "Learn",
    items: [
      { to: "/edition", label: "Edition", icon: "spark" as const },
...
function AppInner() {
  ...
  useEffect(() => {
    document.title = currentLabel
      ...
      : "DevPulse - understand AI engineering";
  ...
  return (
    <div className="min-h-screen flex bg-paper">
      ...
      <main className="flex-1 min-w-0 flex flex-col lg:ml-56">
          ...
          <div className="flex items-center gap-3 min-w-0 flex-1">
            ...
            {!isDesktop ? (
              <NavLink to="/" className="flex items-center gap-2 min-w-0">
                <img src="/icon.svg" alt="" className="hidden sm:block w-5 h-5 shrink-0" />
                <span className="wordmark text-[16px] leading-none shrink-0">
                  <span className="text-ink">Dev</span><span className="text-accent">Pulse</span>
                </span>
                {currentLabel && (
                  <>
                    <span className="hidden sm:block text-ink-faint/40 text-[13px] shrink-0">/</span>
                    <span className="hidden sm:block text-[13px] text-ink-muted truncate">{currentLabel}</span>
                  </>
                )}
              </NavLink>
            ) : (
        ...
        <div id="main" className="flex-1 max-w-5xl w-full mx-auto px-4 sm:px-8 py-6 sm:py-10">
          ...
          <Routes>
            ...
            <Route path="/edition" element={<TodayPage />} />
            ...
            <Route path="/brief" element={<Navigate to="/edition" replace />} />
            <Route path="/knowledge" element={<Navigate to="/archive" replace />} />
            <Route path="/knowledge/:slug" element={<Navigate to="/archive" replace />} />
            <Route path="/learn" element={<Navigate to="/edition" replace />} />
            <Route path="/investigations/*" element={<Navigate to="/edition" replace />} />
